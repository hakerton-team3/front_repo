// answer.js
const answer = [
    {
      id: 1,
      hashTag: "#P#주당#술술술",
      mainDescription: "무계획으로 혼자 많은 양의 술을 즐기는",
      title: "번개 술꾼",
      improvingDescription1: "즉흥적인 음주가 즐거울 수 있지만 계획되지 않은 음주는 건강에 해로울 수 있습니다.",
      improvingDescription2: "미리계획된 술자리를 가지면서 자신을 돌보세요.",
      improvingDescription3: "간혈적인 술 없는 날을 정해 건강을 지키세요.",
    },
    {
      id: 2,
      hashTag: "#규칙#나혼자#술술",
      mainDescription: "주기적으로 계획을 세워 혼자만의 시간에 많은 양의 술을 즐기는",
      title: "알코올 애호가",
      improvingDescription1: "계획적인 음주는 좋지만, 과도한 음주는 건강에 해로울 수 있습니다..",
      improvingDescription2: "주기적으로 술 없는 날을 계획하도록 해봐요.",
      improvingDescription3: "술을 마시는 날에도 적정량을 지키며 자신의 건강을 돌보세요.",
    },
    {
      id: 3,
      hashTag: "#조금만 #P #분위기",
      mainDescription: "즉흥적으로 적은 양의 술을 혼자 즐기는",
      title: "감성 타는 혼술러",
      improvingDescription1: "적은 양의 술을 즐기는 것을 좋지만, 혼자 마시는 시간이 길어지지 않도록 하세요.",
      improvingDescription2: "친구나 가족과의 소통을 늘려 혼술 빈도를 조절해요.",
      improvingDescription3: "감정적인 이유로 음주를 하지 않도록 해보세요.",
    },
    {
      id: 4,
      hashTag: "#계획 #꼼꼼 #조금만",
      mainDescription: "자신만의 시간을 계획을 세워 적은 양의 술을 혼자 즐기는",
      title: "꼬꼬마 술애호가",
      improvingDescription1: "계획적으로 적은 양의 술을 즐기는 것은 좋은 습관입니다.",
      improvingDescription2: "술을 마신 후에는 충분한 수분을 섭취하고 균형 잡힌 식사를 하세요.",
      improvingDescription3: "간자신만의 시간을 다른 취미와 함께 즐겨보는 것도 좋습니다.",
    },
    {
      id: 5,
      hashTag: "#극강의 #프로참석러",
      mainDescription: "지인들과 자주 번개 약속을 가져 많은 양의 술을 즐기는",
      title: "술먹자 빌런",
      improvingDescription1: "자주 모임을 가지는 것은 좋지만, 많은 양의 술을 마시는 것은 유의해 주세요..",
      improvingDescription2: "모임 전후로 충분한 휴식을 취하고, 중간중간 물을 마시며 적정량을 지키세요..",
      improvingDescription3: "모임을 가지는 횟수를 조절해보세요..",
    },
    {
      id: 6,
      hashTag: "#술이 #술술 #들어가",
      mainDescription: "지인들과의 술약속에 많은 양의 술을 즐기는",
      title: "술약속 선도자",
      improvingDescription1: "많은 양의 술을 마시는 것은 건강에 해로울 수 있으니 유의하며 마셔주세요.",
      improvingDescription2: "빈 속 음주는 피하고, 술을 마신 후에는 충분한 수분을 섭취하세요.",
      improvingDescription3: "모임 횟수를 줄이고, 모임 중간에 휴식을 취하며 적정량을 지켜주세요.",
    },
    {
      id: 7,
      hashTag: "#갑자기더라도 #조금만",
      mainDescription: "번개 모임에서 적은 양의 술을 마시는",
      title: "모임 충성론자",
      improvingDescription1: "모임에서 술을 즐기는 것은 좋지만, 과음은 피해야 합니다.",
      improvingDescription2: "모임 중간중간 물을 마시며, 적당한 음주량을 지켜보세요.",
      improvingDescription3: "즐거운 모임 후에 건강을 위해 충분한 휴식을 취하세요",
    },
    {
      id: 8,
      hashTag: "#참을인 #조금만마셔",
      mainDescription: "미리 계획된 약속에서 주로 적은 양의 술을 즐기는",
      title: "절제의 달인",
      improvingDescription1: "자신의 주량을 잘 알고, 항상 적정량을 유지하는 것은 매우 중요합니다.",
      improvingDescription2: "술을 마신 후에 충분한 수분을 섭취하고 균형 잡힌 식사를 하세요.",
      improvingDescription3: "절제를 통해 건강을 유지하며 술을 즐기는 습관을 계속 이어가세요.",
    },
  ];
  
  export default answer;
  